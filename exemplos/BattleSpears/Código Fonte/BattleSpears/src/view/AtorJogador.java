...
public class AtorJogador {
		...
		if(tabuleiro.getJogador1().isJogadorDaVez()){
			...
			if(tabuleiro.getJogador1().isVencedor()){
				...
				if(tabuleiro.getJogador1().getNumeroDeVitorias() < 2){
					this.notificar("Uma nova campanha ser� iniciada, o outro jogador inicia. Aguarde.");
				}else{
			...
			throw new Exception("N�o � sua vez");
		}
	}
	...
	public void receberJogada(Tabuleiro tab) throws NaoConectadoException, NaoJogandoException{
		if (tab == null) {
			int opt = this.tela.perguntar("O outro jogador deseja reiniciar a partida. Voc� iniciar� com o lado branco.");
			if (opt == 0) {
				this.reiniciar();
				return;
			} else {
		...
		if (this.tabuleiro.getJogador2().isVencedor()) {
			...
			if(tabuleiro.getJogador2().getNumeroDeVitorias() < 2){
				this.notificar("Uma nova campanha ser� iniciada, voc� inicia.");
		...
		this.tela.notificar("� a sua vez " + this.tabuleiro.getJogador1().getNome());
	...
	public void iniciarNovaPartida(Integer posicao) {
		...
		tela.limpar();
		tela.notificar("Partida encontrada, o nome de seu advers�rio �: " + idAdversario);
		tela.iniciarPartida.setEnabled(false);
		tela.conectar.setEnabled(false);
		if(tabuleiro.getJogador1().isJogadorDaVez()){
			tela.setaImagemJogador();
		}else{
			this.tela.notificar("Agora � a vez do jogador " +idAdversario+".Aguarde sua jogada");
		}
